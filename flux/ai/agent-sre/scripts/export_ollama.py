#!/usr/bin/env python3
"""
Export fine-tuned FunctionGemma model to Ollama format for Agent-SRE

This script:
1. Merges LoRA adapters with base MLX model using mlx_lm.fuse
2. Creates a Modelfile for Ollama with Agent-SRE specific configuration
3. Prepares model for Ollama import

Usage:
    python export_ollama.py \
        --base-model /path/to/mlx-functiongemma-270m \
        --adapter-path /path/to/adapters \
        --output ./agent-sre-ollama \
...
def create_modelfile(output_path: str, model_name: str, base_model_path: str) -> str:
    """
    Create Modelfile for Ollama with Agent-SRE configuration.
...
def export_to_ollama(
    ...
    instructions = f"""📋 Ollama Import Instructions for Agent-SRE:
...
📋 Using in Agent-SRE LambdaAgent:
   Update k8s/kustomize/base/lambdaagent.yaml:
   ai:
     provider: ollama
     endpoint: "http://ollama-native.ollama.svc.cluster.local:11434"
     model: "{model_name}"

   Or update k8s/kustomize/base/deployment.yaml:
   env:
     - name: MODEL_NAME
       value: "{model_name}"
     - name: MODEL_BACKEND
...
def main():
    parser = argparse.ArgumentParser(
        description="Export fine-tuned FunctionGemma model to Ollama format for Agent-SRE"
    )
    parser.add_argument(
        "--base-model",
        required=True,
