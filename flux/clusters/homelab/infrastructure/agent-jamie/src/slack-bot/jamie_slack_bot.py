...
class AgentSREClient:
    ...
    async def list_tools(self) -> List[Dict]:
        ...
        try:
            ...
            payload = {"jsonrpc": "2.0", "id": self._get_next_request_id(), "method": "tools/list", "params": {}}

            async with self.http_session.post(
                self.mcp_url, json=payload, timeout=aiohttp.ClientTimeout(total=10)
            ) as response:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict:
        ...
        try:
            ...
            async with self.http_session.post(
                self.mcp_url, json=payload, timeout=aiohttp.ClientTimeout(total=30)
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    ...
                    if "error" in data:
                        ...
                        return {"response": f"MCP error: {data['error']['message']}", "error": True}
                    ...
                    return {"response": response_text, "error": is_error, "raw_result": result}
                else:
                    error_text = await response.text()
                    logger.error(f"❌ MCP tool {tool_name} failed: {response.status} - {error_text}")
                    return {"response": f"MCP tool error: {error_text}", "error": True}

        except Exception as e:
            logger.error(f"❌ Error calling MCP tool {tool_name}: {e}", exc_info=True)
            return {"response": f"Error calling MCP tool: {str(e)}", "error": True}
...
Return ONLY the PromQL query, nothing else."""
    ...
    async def prometheus_query(self, query: str, context: Optional[Dict] = None) -> Dict:
        ...
        return {"response": result.get("response"), "error": False}
    ...
    async def chat(self, message: str, context: Optional[Dict] = None) -> Dict:
        ...
        try:
            async with self.http_session.post(
                f"{self.base_url}/chat", json=payload, timeout=aiohttp.ClientTimeout(total=30)
            ) as response:
                if response.status == 200:
                    result = await response.json()
                    ...
                    return {"response": "I'm having trouble connecting to the SRE agent.", "error": True}
        except Exception as e:
            logger.error(f"Agent-SRE Service error: {e}")
            return {"response": "I'm experiencing technical difficulties connecting to the SRE agent.", "error": True}
    ...
    async def k8s_query(self, message: str, context: Optional[Dict] = None) -> Dict:
        ...
        try:
            async with self.http_session.post(
                f"{self.base_url}/k8s/query", json=payload, timeout=aiohttp.ClientTimeout(total=30)
            ) as response:
                if response.status == 200:
                    result = await response.json()
                    ...
                    return {"response": "I'm having trouble querying Kubernetes.", "error": True}
        except Exception as e:
            logger.error(f"Agent-SRE K8s error: {e}")
            return {"response": "I'm experiencing technical difficulties querying Kubernetes.", "error": True}
...
class JamieLangGraphAgent:
    ...
    def __init__(self, agent_sre_client: AgentSREClient, model_name: str = MODEL_NAME):
        ...
        self.llm = ChatOllama(model=self.model_name, base_url=OLLAMA_URL, temperature=0.0)
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            "decide_action", self._route_decision, {"call_tool": "call_tool", "respond": "respond", "end": END}
        )
    ...
    async def _discover_tools_node(self, state: AgentState) -> Dict:
        ...
        return {"available_tools": tools, "next_action": "decide"}
    ...
    async def _decide_action_node(self, state: AgentState) -> Dict:
        ...
        # Note: We use keyword matching instead of LLM for tool selection
        # to keep the agent fast and predictable
        ...
        # Grafana queries
        grafana_keywords = ["grafana", "dashboard", "panel", "visualization", "chart", "graph"]

        # MCP tools discovery
        mcp_keywords = ["mcp", "tools", "list", "discover", "available", "agent-sre"]
        ...
        if matched_prometheus:
            ...
            return {"next_action": "call_tool", "selected_tool": "prometheus_query", "tool_arguments": {"query": query}}
        ...
        if matched_mcp:
            ...
            return {"next_action": "call_tool", "selected_tool": "list_tools", "tool_arguments": {}}
    ...
    async def _call_tool_node(self, state: AgentState) -> Dict:
        ...
        if selected_tool == "list_tools":
            ...
            try:
                ...
                result = {"response": f"⚠️ Error connecting to MCP server: {str(e)}", "error": True}
        else:
            # Call the MCP tool
            try:
                ...
                result = {"response": f"⚠️ Error calling {selected_tool}: {str(e)}", "error": True}

        # Store result
        tool_results = state.get("tool_results", [])
        tool_results.append({"tool": selected_tool, "arguments": tool_arguments, "result": result})
    ...
    async def _respond_node(self, state: AgentState) -> Dict:
        ...
        if tool_results:
            ...
            results_text = "\n\n".join(
                [f"Tool: {r['tool']}\nResult:\n{r['result'].get('response', 'No response')}" for r in tool_results]
            )
            ...
            if any(kw in message_lower for kw in ["prometheus", "grafana", "mcp", "tools", "query", "metrics"]):
                response_text = (
                    "⚠️ I detected a request for SRE tools, but the Agent-SRE MCP server "
...
class JamieSlackBot:
    ...
    async def _handle_general_conversation(self, message: str, context: Optional[Dict] = None) -> str:
        ...
        try:
            ...
            return """🤖 Hi! I'm Jamie, your SRE assistant.
...
What would you like to know? 🚀"""
    ...
    async def handle_api_chat(self, request):
        ...
        try:
            ...
            return web.json_response({"response": response, "timestamp": datetime.now().isoformat()})
    ...
    async def handle_api_prometheus_query(self, request):
        ...
        try:
            ...
            if result.get("error"):
                return web.json_response({"error": result.get("details", "Failed to query Prometheus")}, status=500)

            return web.json_response({"result": result.get("response"), "timestamp": datetime.now().isoformat()})
    ...
    async def handle_api_golden_signals(self, request):
        ...
        try:
            ...
            if not service:
                return web.json_response({"error": "Service name is required"}, status=400)
    ...
    async def handle_api_pod_logs(self, request):
        ...
        try:
            ...
            if not pod_name:
                return web.json_response({"error": "Pod name is required"}, status=400)
            ...
            context = {"pod_name": pod_name, "namespace": namespace, "lines": lines}
            if container:
            ...
            if result.get("error"):
                return web.json_response({"error": result.get("response", "Failed to get pod logs")}, status=500)
    ...
    async def handle_api_analyze_logs(self, request):
        ...
        try:
            ...
            return web.json_response({"analysis": analysis, "timestamp": datetime.now().isoformat()})
    ...
    async def handle_api_ready(self, request):
        ...
        try:
            ...
            if status.get("status") != "healthy":
                return web.json_response({"status": "not_ready", "reason": "Agent-SRE unavailable"}, status=503)

            return web.json_response(
                {"status": "ready", "service": "jamie-slack-bot", "timestamp": datetime.now().isoformat()}
            )
    ...
    async def handle_api_status(self, request):
        ...
        try:
            ...
            return web.json_response(
                ...
                status=200,
            )  # Return 200 even if degraded, so Homepage knows Jamie is alive
    ...
    def _setup_handlers(self):
        ...
        async def handle_mention(event, say, client):
            ...
            try:
                await client.chat_postMessage(channel=channel, text=f"{self.bot_emoji} _Thinking..._")
            except Exception:
                pass
...
_All systems operational!_
    ...
    def _get_user_context(self, user_id: str) -> Optional[Dict]:
        ...
        return {"conversation_history": history, "last_updated": context_data.get("last_updated")}

    def _update_user_context(self, user_id: str, question: str, response: str):
        """Update user's conversation context"""
        if user_id not in self.user_contexts:
            self.user_contexts[user_id] = {"history": [], "last_updated": datetime.now(UTC).isoformat()}
...
async def main():
    ...
    await bot.run_rest_api(api_host, api_port)
